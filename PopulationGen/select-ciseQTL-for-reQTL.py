...
out = open('eGene-topSNP.txt','w')
for x in cdict1:
	for y in cdict1[x]:
		out.write(x+'\t'+y+'\t'+str(cdict[x][y])+'\tControl\n')
for x in hdict1:
	for y in hdict1[x]:
		out.write(x+'\t'+y+'\t'+str(hdict[x][y])+'\tHeat\n')
out.close()
